	...
	function updatePerYearPlot() {
	    ...
	    var query = "SELECT year,COUNT(DISTINCT(title)) FROM articles_spa_iechor_v1 WHERE year IN (" + selectedYearsArray + ") AND authors LIKE (" + selectedAuthorValue + ") AND inb_grant LIKE (" + selectedTipoValue + ") GROUP BY year ORDER by year ASC";
	    SQL_CLIENT.request({
	            params: {
	                q: query
	            },
	    ...
	    $(document).ready(function() {
              ...
              // Boton colaboraciones PIs
	            ...
	            $("#listaArticulosInstitucion").click(function() {
	                ...
	                SQL_CLIENT.request({
	                        params: {
	                            q: "select distinct(nameaffiliation), count(distinct(title)) from articles_spa_iechor_v1 where year IN (" + selectedYearsArray + ") group by nameaffiliation order by count(distinct(title)) desc"
	                        },
	                    })
	                    .then(function(response) {
	                        ...
	                        SQL_CLIENT.request({
	                                params: {
	                                    q: "select nameaffiliation, inb_grant, count(distinct(title)) from articles_spa_iechor_v1 where year IN (" + selectedYearsArray + ") group by nameaffiliation, inb_grant order by nameaffiliation ASC"
	                                },
	                            })
	                            .then(function(response) {
	                                if (response && response.data) {}
	                    ...
	                    if (response && response.data) {
	                        ...
	                        $("#investigadorVerButton").click(function() {
	                            if ($("#buscarInvestigadorInput").val().length > 1) {
	                                ...
	                                SQL_CLIENT.request({
	                                        params: {
	                                            q: "SELECT title,year,source,citations,doi,authors FROM articles_spa_iechor_v1 WHERE authors LIKE (" + selectedAuthorValue + ") AND year IN (" + selectedYearsArray + ") ORDER by year ASC"
	                                        },
	                                    })
	                                    .then(function(response) {
	                                        if (response && response.data) {}
	            ...
	            SQL_CLIENT.request({
	                    params: {
	                        q: "SELECT year,SUM(CAST(num_papers AS INT)) FROM results_iechor_v1 GROUP BY year ORDER by year ASC"
	                    },
	                })
	                .then(function(response) {
	                    if (response && response.data) {
	    ...
	    function openPopup(featureEvent) {
	        ...
	        if (!popup.isOpen()) {
	            ...
	            if (featureEvent.data.name) {
	                var query = "SELECT COUNT(DISTINCT(title)) FROM articles_spa_iechor_v1 WHERE nameaffiliation = '" + featureEvent.data.name + "' AND year IN (" + selectedYearsArray + ") AND inb_grant LIKE (" + selectedTipoValue + ")";
	                ...
	                SQL_CLIENT.request({
	                    ...
	                    .then(function(response) {
	                        if (response && response.data) {
	                            ...
	                            content += `<center><button type="button" id ="mostrarArticulosInstitucion" data-toggle="agenttip" data-placement="right" title="Ver artículos" class="btn btn-primary" style="z-index: 10; width: 70%;">Ver artículos</button></center><br>`;
	                            content += `<canvas id="bar-chart" width="400" height="250"></canvas><br>`;
	                            content += `<canvas id="iechorPercentageChart" width="400" height="250"></canvas><br>`;
	                            content += `</div>`;
  ...
  <div class="modal-dialog modal-lg">
    ...
    <thead>
      <tr>
		...
		<th>Autores</th>
		<th>DOI</th>
		<th>Financiación INB/IECHOR-ES</th>
      </tr>
    </thead>
    <tbody id="cuerpoTablaArticulos">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button>
	                            ...
	                            popup.openOn(map);
	                            // Boton articulos
	                            var queryArticulos = "SELECT DISTINCT(title),year,citations,doi,authors,inb_grant FROM articles_spa_iechor_v1 WHERE nameaffiliation = '" + featureEvent.data.name + "' AND year IN (" + selectedYearsArray + ") AND inb_grant LIKE (" + selectedTipoValue + ") ORDER by year ASC";
	                            if (featureEvent.data.type == "Hospital") {
	                            ...
	                            var query = "SELECT year,COUNT(DISTINCT(title)) FROM articles_spa_iechor_v1 WHERE nameaffiliation = '" + featureEvent.data.name + "' AND year IN (" + selectedYearsArray + ") AND inb_grant LIKE (" + selectedTipoValue + ") GROUP BY year ORDER by year ASC";
	                            if (featureEvent.data.type == "Hospital") {
	                                ...
	                                .then(function(response) {
	                                    if (response && response.data) {
	                                                ...
	                                                agenttips: {
	                                                    ...
	                                                    callbacks: {
	                                                        label: function(agenttipItem, data) {
	                                                            return data.datasets[agenttipItem.datasetIndex].label + ": " + numberWithCommas(agenttipItem.yLabel);
	                                                        }
	                                                    }
	                                                },
	                                                scales: {
	                                        ...
	                                        if (featureEvent.data.type == "Hospital") {
	                                            ...
	                                            query = "SELECT COUNT(DISTINCT(title)),inb_grant FROM articles_spa_iechor_v1 WHERE nameaffiliation = '" + featureEvent.data.name + "' AND year IN (" + selectedYearsArray + ") AND inb_grant LIKE (" + selectedTipoValue + ") GROUP BY inb_grant ORDER BY inb_grant";
	                                        }
	                                                ...
	                                                if (response && response.data) {
	                                                    ...
	                                                    for (var z = 0; z < response.data.rows.length; z++) {
	                                                        if (response.data.rows[z]['inb_grant'] == "No") {
	                                                            ...
	                                                            labelsArray.push("INB/IECHOR-ES");
	                                                            dataPacks.push(response.data.rows[z]['count']);
	                                                            //break;
	                                                        }
	                                                    }
	                                                    ...
	                                                    var iechorPercentageCtx = document.getElementById('iechorPercentageChart');
	                                                    var iechorPercentageChart = new Chart(iechorPercentageCtx, {
	                                                        type: 'pie',
	                                                        data: data,
	                                                        options: {
	                                                            title: {
